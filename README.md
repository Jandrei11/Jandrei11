# Hi, I'm Francis!

## About Me

I'm an **incoming freshman** at the **University of the Philippines Los Banos** pursuing a **BS in Computer Science** degree. I'm passionate right now in building my foundations in programming as someone who has little to no experience while looking to explore cloud technologies and DevOps once I get the hang of this. I believe in learning by doing, and I'm excited to turn ideas into code and solutions.

With a growing interest in software development, I'm on a mission to understand how technology can solve real-world problems. Still early in my coding journey, but eager to learn, create, and improve every single day.

---

## Current Skills

### Programming
- ![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white) **Python** - Building my foundations

### Specialties
-  **Problem Solving** - Breaking down complex issues into manageable parts

### Technologies I'm Exploring
![Git](https://img.shields.io/badge/-Git-F05032?style=flat-square&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/-Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![Cloud](https://img.shields.io/badge/-Cloud%20Tech-FF9900?style=flat-square&logo=amazon-aws&logoColor=white)

---

## Learning Path

I'm actively building my skills in:
- 📚 **Python Programming** - From basics to intermediate
- ☁️ **Cloud Technologies** - Understanding AWS, Azure, and deployment (currently have no idea)
- 🔧 **DevOps Fundamentals** - Containers, automation, and infrastructure (currently have no idea)
- 🌐 **Web Development** - Next on my learning roadmap 

---

## My First Project

**Investigatory Project** (Vibe-Coded Edition)
- My first hands-on coding experience that further sparked my interest in tech
- Worked with embedded systems, integrating arduino with IoT functionalities
- A great learning experience that showed me the power of creative problem-solving

*More projects coming soon as I progress through my CS program!*

---

## 🌱 Currently Learning

- **Python Fundamentals** - Strengthening my programming basics
- **DevOps & Cloud Concepts** - Understanding modern development practices
- **Best Coding Practices** - Writing clean, maintainable code
- **Open Source Contributions** - Getting involved with the community

---

## 🎓 Education

**UP Los Banos**  
BS Computer Science (Class of 2029)  
*Freshman - 2026*

---

## 💬 Let's Connect

I'd love to collaborate, share ideas, or just chat about tech! Feel free to reach out:

- **LinkedIn**: [Francis Munoz](https://www.linkedin.com/in/francis-munoz-7b02ba23a/?isSelfProfile=true)
- **Instagram**: [@frfrfrfrancis](https://www.instagram.com/frfrfrfrancis/)
- **GitHub**: [@Jandrei11](https://github.com/Jandrei11)

---

## 🎯 My Goals

- ✅ Build a strong foundation in computer science fundamentals
- ✅ Contribute to open-source projects
- ✅ Create meaningful projects that solve real problems
- ✅ Master cloud technologies and DevOps practices
- ✅ Grow as a developer and tech professional

---

**Last Updated**: May 29, 2026
